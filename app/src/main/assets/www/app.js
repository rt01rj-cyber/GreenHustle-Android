...
function art(name,extra){if(window.HotBoxComic&&HotBoxComic.has(name))return HotBoxComic.draw(name,extra||'');if(window.HotBoxArt&&HotBoxArt.has(name))return '<div class="art fresh '+(extra||'')+'">'+HotBoxArt.draw(name)+'</div>'; var car=name==='car'?'<svg viewBox="0 0 200 120" aria-hidden="true"><path d="M17 83l12-30 31-5 21-24h54l27 26 18 6 6 33-16 6H29z" fill="#263d50" stroke="#55ddf9" stroke-width="3"/><path d="M67 48l19-18h45l19 19z" fill="#07121d" stroke="#56b7c9" stroke-width="2"/><circle cx="47" cy="90" r="15" fill="#060b12" stroke="#a4c6d6" stroke-width="5"/><circle cx="158" cy="90" r="15" fill="#060b12" stroke="#a4c6d6" stroke-width="5"/><path d="M24 65h25l-4 9H23M160 60l17 7-3 8h-20" fill="#e9ffb0"/><path d="M90 73h48v11H90z" fill="#ece8b9"/></svg>':'';return '<div class="art art-'+esc(name)+(extra?' '+extra:'')+'">'+car+'</div>';}
...
function portrait(p){return art(p.hero||AV[p.avatar]||'you','avatar');}
...
function save(){if(window.AHUI&&AHUI.active()){AHUI.save(state);return;}if(state)try{localStorage.setItem(KEY,JSON.stringify(state));}catch(e){notify('Storage is unavailable. Keep the game open to retain this match.');}}
...
function renderRival(p,i){var active=i===E.seat(state),st=E.sum(p.stash);return '<button class="rival '+(active?'active ':'')+(p.raid?'raided':'')+'" data-do="rival" data-seat="'+i+'" aria-label="Inspect '+esc(p.name)+'"><div class="rival-top">'+portrait(p)+'<div class="rival-info"><strong>'+esc(p.name)+'</strong><div class="cash">'+E.money(p.bank)+'</div></div></div><small>'+esc(p.style)+'</small><div class="status-line">'+(p.line?'LINE ON':'LINE OFF')+' · '+p.hand.length+' cards'+(p.raid?' · RAID':'')+(p.fake?' · ½':'')+'</div><div class="rival-products">'+(p.stash.length?p.stash.slice(0,6).map(function(c){return '<div class="micro-card">'+art(E.def(c).art)+'<span>'+E.money(E.value(c))+'</span></div>';}).join(''):'<span class="muted" style="font-size:9px">No exposed stash</span>')+'</div></button>';}
function renderHand(p,opts,own){
 var groups=[];p.hand.forEach(function(c){var g=groups.find(function(x){return x.card.id===c.id;});if(g)g.count++;else groups.push({card:c,count:1});});
 if(prefs.sort==='value')groups.sort(function(a,b){return E.value(b.card)-E.value(a.card);});
 else if(prefs.sort==='type')groups.sort(function(a,b){return E.def(a.card).kind.localeCompare(E.def(b.card).kind);});
 return groups.map(function(g){var c=g.card,yes=opts.some(function(a){return a.type==='play'&&a.uid===c.uid;}),html=card(c,false,'inspect');
 html=html.replace('class="card','class="card '+(yes?'available':own&&state.phase==='act'?'unavailable':'')+' ');
 return html.replace('</button>',(g.count>1?'<span class="duplicate-count">×'+g.count+'</span>':'')+'</button>');}).join('');
}
function renderGame(){
 var oldHand=root.querySelector('.hand'),scroll=oldHand?oldHand.scrollLeft:0;
 var idx=myIndex(),p=mine(),seat=E.seat(state),turn=state.players[seat],own=seat===idx&&turn.human,opts=E.legal(state,idx),ev=state.events.find(function(x){return x.type!=='draw';})||state.events[0],danger=state.deck.length<=Math.ceil(state.initialDeck/3),last=state.discard[state.discard.length-1];
 var label=state.phase==='trim'?(own?'CHOOSE EXCESS CARDS TO BIN':turn.name.toUpperCase()+' SORTS THEIR HAND'):state.phase==='react'?(own?'YOUR RESPONSE':turn.name.toUpperCase()+' RESPONDS'):own?(state.phase==='draw'?'YOUR TURN · DRAW':'YOUR TURN · ONE ACTION'):turn.name.toUpperCase()+' IS PLAYING';
...
function render(){document.body.classList.toggle('reduced',!prefs.motion);if(screen==='menu'){renderMenu();if(window.AHUI)AHUI.menu();return;}
 ...
 renderGame();if(window.AHUI&&AHUI.active())AHUI.decorate(state);
 if(locked){showHandoff();return;}
 handoff.hidden=true;
 if(state.phase==='round'||state.phase==='over'){if(window.AHUI&&AHUI.active()){AHUI.complete(state);return;}if(modal!=='results')showResults();return;}
 if(state.phase==='trim'&&state.players[E.seat(state)].human){showTrim();return;}
...
function continueGame(){if(window.AHUI)AHUI.detach();var s=saved();if(!s){notify('No compatible save is available. Start a new table.');return;}stop();state=s;screen='game';viewSeat=null;locked=s.mode==='local'&&s.phase!=='round'&&s.phase!=='over';modal='';overlay.hidden=true;paused=false;render();}
function home(){stop();save();if(window.AHUI)AHUI.detach();screen='menu';modal='';overlay.hidden=true;handoff.hidden=true;locked=false;render();}
function showModal(title,html,type,extra){stop();modal=type||'info';overlay.hidden=false;overlay.innerHTML='<section class="sheet '+(extra||'')+'" role="dialog" aria-modal="true" aria-label="'+esc(title)+'"><header class="sheet-head"><h2>'+esc(title)+'</h2>'+(['results','reaction','trim'].indexOf(modal)<0?'<button class="icon-button" data-do="close" aria-label="Close">'+icon('close')+'</button>':'')+'</header>'+html+'</section>';}
...
function hitPreview(){
 var q=state.players[state.pending.to],c=state.pending.card;
 if(c.id==='raid')return 'No selling or banking for two turns. You can still draw and counter.';
 var pool=(q.stash.length?q.stash:q.hand).filter(function(x){return E.value(x)>0;}).sort(function(a,b){return E.value(b)-E.value(a);});
 if(!pool.length)return 'You have no product to lose. Your bank remains safe.';
 var x=pool[0];if(q.stash.length&&x.id==='baggie'&&q.mods&&q.mods.cushion&&!q.cushionUsed)return 'Cwtch Cushion will protect your exposed £5k product.';
 return 'Lose '+E.def(x).name+' worth '+E.money(E.value(x))+'. Banked cash stays safe.';
}
function showReaction(){
 var a=state.pending,p=state.players[a.to],d=E.def(a.card),opts=E.legal(state,a.to),nc=opts.find(function(x){return x.card==='nocomment';}),dash=opts.some(function(x){return x.card==='dash';});
 var html='<div class="reaction-summary">'+art(d.art,'reaction-art')+'<div><b>'+esc(state.players[a.from].name)+' played '+esc(d.name)+'</b><p>'+esc(hitPreview())+'</p></div></div><div class="sheet-actions">';
 html+=nc?performButton('No Comment · cancel',nc,'primary'):button('No Comment · not in your hand','none','quiet','disabled');
 html+=button('Dash It · choose a sacrifice','dashpick',dash?'':'quiet',dash?'':'disabled');
 html+=performButton('Take the hit',{type:'accept'},'danger')+'</div><p class="reason">No timer. Your response does not spend your next turn.</p>';
 showModal(p.name+' · your response',html,'reaction');
}
function showDashPicker(){
 var p=state.players[state.pending.to],options=E.legal(state,state.pending.to).filter(function(a){return a.card==='dash';}),groups=[];
 options.forEach(function(a){var c=p.hand.find(function(x){return x.uid===a.sacrifice;}),g=groups.find(function(x){return x.card.id===c.id;});if(g)g.count++;else groups.push({a:a,card:c,count:1});});
 var html='<p class="help">Discard Dash It and ONE product below. The police hit is cancelled.</p><div class="sacrifice-grid">'+groups.map(function(g){return '<button class="sacrifice" data-do="perform" data-action="'+esc(JSON.stringify(g.a))+'">'+art(E.def(g.card).art)+'<b>'+esc(E.def(g.card).name)+'</b><small>Lose '+E.money(E.value(g.card))+' · '+g.count+' held</small></button>';}).join('')+'</div><div class="sheet-actions">'+button('Back to responses','reactionback','quiet')+'</div>';
 showModal('Dash It · sacrifice one',html,'reaction');
}
function showTrim(){
 var p=state.players[state.current],opts=E.legal(state,state.current),groups=[];
 opts.forEach(function(a){var c=p.hand.find(function(x){return x.uid===a.uid;}),g=groups.find(function(x){return x.card.id===c.id;});if(g)g.count++;else groups.push({a:a,card:c,count:1});});
 var html='<p class="help">Story hand limit: 7. Choose '+(p.hand.length-7)+' card'+(p.hand.length-7===1?'':'s')+' to bin. This does NOT use your main action. No cards are discarded automatically.</p><div class="sacrifice-grid">'+groups.map(function(g){return '<button class="sacrifice" data-do="perform" data-action="'+esc(JSON.stringify(g.a))+'">'+art(E.def(g.card).art)+'<b>'+esc(E.def(g.card).name)+'</b><small>'+g.count+' held · bin one</small></button>';}).join('')+'</div>';
 showModal('Make room in your hand',html,'trim');
}
...
document.addEventListener('click',function(e){var b=e.target.closest('[data-do]');if(!b||b.disabled)return;var cmd=b.dataset.do;
 if(cmd==='sorthand'){prefs.sort=prefs.sort==='value'?'type':prefs.sort==='type'?'dealt':'value';storePrefs();render();notify('Hand sorted: '+prefs.sort);}
 else if(cmd==='dashpick')showDashPicker();
 else if(cmd==='reactionback')showReaction();
 else if(cmd==='mode'){readSetup();mode=b.dataset.value;render();}
...
overlay.addEventListener('click',function(e){if(e.target===overlay&&modal!=='reaction'&&modal!=='results'&&modal!=='trim')closeModal();});
...
function back(){if(window.AHUI&&AHUI.back&&AHUI.back())return true;if(locked){home();return true;}if(modal){if(modal==='reaction'||modal==='results'||modal==='trim')return true;closeModal();return true;}if(screen==='game'){document.querySelector('[data-do="pausemenu"]').click();return true;}return false;}
